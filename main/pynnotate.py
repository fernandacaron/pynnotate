...
def extract_genes(record, header_id, alias_map, genes_selected, seen_genes, gene_dict, aliases_unknown, aliases_duplicated, group_leu, group_ser, seen_leu_ser, species_to_genes=None, organism=None):
...
def metadata_gene_vs_amostra(gene_dict, excel_data, header_fields, output_dir):
	...
	meta_por_id = {
		genbank_id: {field: values.get(field, '') for field in header_fields}
		for genbank_id, values in header_map.items()
	}
		...
		for _, _, genbank_id in seqs:
			if genbank_id in meta_por_id:
				if genbank_id not in samples:
					samples[genbank_id] = {
						"metadata": meta_por_id[genbank_id],
	...
	wb = Workbook()
	ws = wb.active
	ws.title = "metadata_gene_vs_amostra"
	ws.append(header_fields + genes)

	ordered = sorted(
		samples,
		key=lambda x: samples[x]["metadata"].get("organism", "").lower()
	)
...
def make_query(email, alias_map, org_type, genes, organisms, mito, mitogenome, chloroplast, title, additional, min_len, max_len, add_unverified_exclusion):
	...
	try:
		ids = search_genbank(email, query, retmax=100000)
	except Exception as e:
		try:
...
def download_sequences(email, genbank_ids, output_dir, header_fields, alias_map, config, genes, query=None, root=None):
	...
	try:
		...
		for record in all_records:
			...
			if unique_species and not new_genes:
				removal_reasons[record_id].append("All genes already saved for species (unique_species enabled)")
				continue
			
			#if unique_species and organism in species_seen:
			#	removal_reasons[record_id].append("Duplicate by species (unique_species enabled)")
			#	continue
			...
			if extract_all_genes:
				print(species_to_genes[organism])
			...
			if not extract_all_genes or genes_found:
				...
				removal_reasons[record_id].append("Gene extraction failed: No genes extracted")
				continue
		...
		with open(processing_file, "w") as pf:
			pf.write("PYNNOTATE - PROCESSING OF SEQUENCES\n")
			pf.write("\n")
			pf.write(f"Processing completed for {len(filtered_records)} samples.\n")
			pf.write(f"Total species saved: {len(species_seen)}\n")
			pf.write("\n")
			
			if unknown_aliases:
				pf.write("[UNKNOWN ALIASES] Some aliases were not detected in the synonyms dictionary and were not extracted:\n")
				for alias in sorted(unknown_aliases):
					origins = ", ".join(sorted(unknown_aliases[alias]))
					pf.write(f"{alias} (from records: {origins})\n")
				pf.write("\n")

			if duplicated_aliases:
				pf.write("[DUPLICATED ALIASES]:\n")
				for alias in sorted(duplicated_aliases):
					pf.write(alias + "\n")
			pf.write("\n")

			if extract_all_genes and gene_dict:
				pf.write("[EXTRACT GENES SUMMARY]:\n")
				for gene in sorted(gene_dict):
					...
					with open(path, "w") as out:
						for head, seq, *_ in entries:
							out.write(f">{head}\n{seq}\n")
					pf.write(f"{gene}: {len(entries)} extracted sequences\n")
				
				if "TRNA-LEU" in gene_dict or "TRNA-Ser" in gene_dict:
					pf.write("\n[TRNA-LEU/SER GROUPINGS BY POSITIONAL PATTERN]\n")
					combined_groupings = list(grouper_leu.items()) + list(grouper_ser.items())
					for tag, entries in sorted(combined_groupings):
						pf.write(f"{tag}: {len(entries)} grouped sequences\n")
			pf.write("\n")

			if delete_overlap and org_type == "animal_mito":
				if not overlap_records:
					pf.write("[FIXING OVERLAP BETWEEN EXTRACTED GENES ENABLED] No overlap found!\n")
				else:
					pf.write("[FIXING OVERLAP BETWEEN EXTRACTED GENES ENABLED] Here are the bp removed:\n")
					key = f"{overlap_name} ({overlap_header})"
					pf.write(f"[{key}]\n")
					for g1, s1, e1, g2, s2, e2, o in overlap_records:
						pf.write(f"⚠️ {g1} ({s1}..{e1}) overlap {g2} ({s2}..{e2}): {o} bp removed from the end of {g1}\n")
				pf.write("\n")

			if log_missing and genes_selected_original:
				pf.write("[MISSING GENES] Some records did not present the genes of interest:\n")
		...
		with open(log_file, "w") as lf:
			...
			lf.write("\n")
			lf.write(f"Total sequences found (before filters): {len(all_records)}\n")
			...
			if delete_overlap:
				lf.write("[FIXING OVERLAP BETWEEN EXTRACTED GENES ENABLED]\n")
			if unique_species:
				lf.write("[KEEPING ONLY ONE INDIVIDUAL PER SPECIES ENABLED]\n")
			lf.write("\n")
			if removal_reasons:
				lf.write(f"[🔎 SEQUENCES REMOVED BY APPLIED FILTERS]: {len(all_records) - len(filtered_records)}\n")
				for rec_id, reasons in removal_reasons.items():
					reasons_str = "; ".join(reasons)
					lf.write(f"{rec_id}: {reasons_str}\n")
			else:
		...
		report_message(f"Download complete! Files saved based on:\n{output_dir}", "info", "Success", root)

		#if extract_all_genes and len(gene_dict) > 1 and len(all_records) > 1:
		#	metadata_by_gene(gene_dict, excel_data, header_fields, output_dir)
		...
		if extract_all_genes and len(gene_dict) > 1 and len(all_records) > 1:
			metadata_gene_vs_amostra(gene_dict, excel_data, header_fields, output_dir)
